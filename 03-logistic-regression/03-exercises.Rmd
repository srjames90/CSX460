...
fit.full.back <- stepAIC(fit.full, scope = ~1, direction = "backward")
...
No, they are not the same model. Forward started with the intercept and added variables as it saw fit while backwards did the reverse. The end result were 2 different models: specifically, backwards took into account variables that forward didn't (e.g., AirAspirationMethod, TransCreeperGear, etc.) and ignored IntakeValveCyl.The backward model is better according to our loss functions, as seen above. The errors for backward are less than those for forward, so one can argue it is marginally better; however, given that both models used the full set of data rather than separating between training data and testing data, I'd say that these models are overfit.
...
credit.glm <-glm(formula = Class ~ 1, family  = 'binomial', data = GermanCredit)

credit.glm.forward <-stepAIC(credit.glm, data = GermanCredit, direction = 'forward')

summary(credit.glm.forward)

credit.glm.pred <- predict(credit.glm.forward, GermanCredit)
...
feature_removal <- function(cor_matrix, threshold = .8) {
  ...
  if(max(cor_matrix) <= threshold)
    return(cor_matrix)
  #find pairs whose abs correlation is equal to the max. Use first max.
  max <- which(max(cor_matrix) == cor_matrix, arr.ind = T)[1,]
  #Get A and B
  a = max[[1]]
  b = max[[2]]
  #Compute the averages
  a_mean = mean(cor_matrix[,-b][a,])
  b_mean = mean(cor_matrix[-a,][,b])
  if(a_mean > b_mean) {
    feature_removal(cor_matrix[-a,])
  } else {
    feature_removal(cor_matrix[,-b])
  }
